...
RegisterCommand("testujupicu", function()
local pica = exports['dyny_bleeding']:GetStressLevel()
	exports['mythic_notify']:SendAlert('error', pica)
end)
...
AddEventHandler('sw_hud:updateStress', function(newStress)
	PlayerStamina2 = GetPlayerSprintStaminaRemaining(PlayerId())
end)
...
Citizen.CreateThread(function()
    while true do
		Citizen.Wait(4)
		if(showhud) then
			local PlayerStress = exports['dyny_bleeding']:GetStressLevel() * 100
